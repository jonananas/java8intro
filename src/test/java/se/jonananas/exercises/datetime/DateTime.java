...
public class DateTime {
	...
	private String lengthOfMonthPerYear(Year year) {
		String daysPerMonth = Stream.of(Month.values()) //
	...
	private List<LocalDate> allMondaysOfMonth(YearMonth month) {
		LocalDate date = month.atDay(1).with(firstInMonth(MONDAY));

		List<LocalDate> result = new ArrayList<>();
		while (date.getMonth().equals(month.getMonth())) {
			result.add(date);
